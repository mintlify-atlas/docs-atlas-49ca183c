...
description: "Reference for Absolet's moderation slash commands: ban (with unban/list), kick, warn (with remove/list), clear, nuke, and announce."
---

Absolet's moderation commands give staff teams precise control over member discipline and channel hygiene. All case-based commands (`/ban`, `/kick`, `/warn`) automatically track case numbers in the database, making audit history easy to review with the `list` subcommand. The `/clear`, `/nuke`, and `/announce` commands are scoped to **Admin** permission.

---

## `/ban`

Ban manager for adding, removing, and viewing bans.
...
Kick manager for removing, tracking, and reviewing kicks.
...
Warning manager for issuing, removing, and listing member warnings.
...
Deletes all messages in a channel by cloning the channel and removing the original, preserving the channel's position. Requires confirmation before proceeding.
...
Sends a custom message or embed to a channel using raw JSON code from an embed builder (e.g., [embed.strider.top](https://embed.strider.top/)).
