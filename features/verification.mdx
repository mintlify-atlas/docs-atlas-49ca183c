...
description: "Absolet protects your server with a verification gate. Choose between CAPTCHA image challenges or one-click direct verification, then assign a role on success."
---
...
The CAPTCHA method generates a unique **450 × 150 px PNG image** using `@napi-rs/canvas` with a random 7-character uppercase word rendered in the custom `deToks` font on a white background. The user is shown the image alongside **five buttons**, each labelled with a different 7-character word string. Only one button matches the word in the image.
